  ...
  useEffect(() => {
    async function fetchDataQuality() {
      try {
        ...
        const res = await fetch("/api/negocio/data-quality-score", {
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center", color: "var(--muted)" }}>
          <div style={{ fontSize: 12, animation: "pulse 1.5s infinite" }}>Cargando...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        lineHeight: 1,
        marginBottom: 8,
      }} aria-label={`Puntuación de calidad: ${data.score}%`}>
        {data.score}%
      </div>
